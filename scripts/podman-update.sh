...
# Monthly Podman container update with inventory validation
# Designed for unprivileged LXC where sudo podman is required.

set -e

# === SUDO CREDENTIAL CACHE ===
# Request sudo password once upfront so the rest runs unattended.
...
detect_stack_type() {
    ...
    if [[ -f "$stack_path/Dockerfile" ]] || [[ -f "$stack_path/Containerfile" ]]; then
        echo "build"
        return
    fi

    # Check compose file for build: section
    local compose_file=""
...
#   build - Has Containerfile/Dockerfile that needs building
#
# Flag options:
...
# custom-app|/opt/stacks/custom-app|Custom built from Containerfile
...
for stack in "${!STACKS_TO_UPDATE[@]}"; do
    ...
    if [[ "$type" == "build" ]]; then
        # Containerfile/Dockerfile-based stack
        info "Rebuilding from Containerfile (pulling fresh base images)..."
        if sudo podman compose build --pull 2>&1 | tee -a "$LOG_FILE"; then
            info "Recreating containers..."
            sudo podman compose up -d --build 2>&1 | tee -a "$LOG_FILE"
        ...
        if sudo podman compose pull 2>&1 | tee -a "$LOG_FILE"; then
            info "Recreating containers..."
            sudo podman compose up -d 2>&1 | tee -a "$LOG_FILE"
            success "Stack updated successfully"
        else
...
for container in "${INVENTORY_CONTAINERS[@]}"; do
    ...
    flags="${INVENTORY_FLAGS[$container]}"
    status=$(sudo podman inspect -f '{{.State.Status}}' "$container" 2>/dev/null || echo "not_found")
...
if [[ ${#FAILED_CONTAINERS[@]} -gt 0 ]]; then
    alert_title="Podman Update Failed"
...
sudo podman image prune -f 2>&1 | tee -a "$LOG_FILE"
