...
# Auto-generate Docker inventory from running containers
...
DESCRIPTION:
    Automatically generates a Docker inventory file by scanning running containers.
    Extracts container names and stack paths from Docker Compose labels.
...
# === MAIN ===
info "Scanning running Docker containers..."
echo ""
...
while IFS= read -r container; do
    ...
    if [[ -n "$working_dir" && "$working_dir" != "<no value>" ]]; then
        ...
        warn "$container → Path not found (not managed by Docker Compose?)"
    fi
...
if [[ ${#UNKNOWN_PATHS[@]} -gt 0 ]]; then
    ...
    info "These containers may not be managed by Docker Compose"
    info "You'll need to manually set their paths in the inventory file"
    echo ""
fi

# === GENERATE INVENTORY CONTENT ===
INVENTORY_CONTENT="# ~/etc/docker-inventory
...
echo "  4. Run validation: $SCRIPT_DIR/validate-inventory.sh"
